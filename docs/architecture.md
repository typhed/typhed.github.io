...
| A shared package, import path, or tsconfig | [Shared Packages Consumed from Source](#shared-packages-consumed-from-source) | [packages/](../packages) |
| A color, a token, or a themed surface | [Theming & Color Tokens](#theming--color-tokens) | [colors.yml](design/colors.yml) |
| Spacing, layout rhythm, or type | [spacing.yml](design/spacing.yml), [typography.yml](design/typography.yml) | [tailwind.config.js](../packages/config-tailwind/tailwind.config.js) |
| Brand copy, dates, nav, or contact links | [Content Single Source of Truth](#content-single-source-of-truth) | [constants.ts](../packages/ui/lib/constants.ts) |
...
```text
  ...
  packages/config-tailwind ----+---> apps/web --[ next build ]--> apps/web/out/
  packages/ui -----------------+                                       |
                                                                       |
...
| Path | Role | Built? |
| :---: | --- | :---: |
| [apps/web](../apps/web) | The public site. Owns routing, metadata, static assets, and the auth wiring. | Yes |
| [packages/ui](../packages/ui) | Shared React components and shared copy, published as `@typhed/ui`. | No, compiled from source |
| [packages/config-tailwind](../packages/config-tailwind) | The Tailwind preset every app extends (`@typhed/tailwind-config`). | No |
| [packages/config-typescript](../packages/config-typescript) | The three tsconfig bases (`@typhed/tsconfig`). | No |
| [permalink](../permalink) | The Python redirect generator, its TOML link table, and the Jinja template. | Yes, separately |
| [docs](.) | This reference set: architecture, components, and the design system (colors, spacing, typography). | No |
| [.github/workflows](../.github/workflows) | The release-triggered Pages deployment. | No |

Workspace membership comes from [pnpm-workspace.yaml](../pnpm-workspace.yaml), which globs `apps/*` and `packages/*`.
A new directory under either is picked up with no further registration. That file also declines three postinstall build
scripts on purpose: `sharp` (the export never optimizes images), `unrs-resolver` (the lint import resolver works without
its native binary), and `@clerk/shared` (its postinstall only prints a telemetry notice).
...
Imports follow the export map in [packages/ui/package.json](../packages/ui/package.json):

```tsx
import { SiteHeader } from "@typhed/ui/components/site-header"
import { Button } from "@typhed/ui/components/ui/button"
import { SITE, NAV_LINKS } from "@typhed/ui/lib/constants"
```
...
| `react-library.json` | [packages/ui](../packages/ui/tsconfig.json) | `jsx: "react-jsx"`, `noEmit` |

`@typhed/tailwind-config` is the shared preset. It carries the theme (colors, radii, fonts, keyframes, animations) and
deliberately ships an **empty `content` array**. Each app supplies its own globs, and
[apps/web/tailwind.config.ts](../apps/web/tailwind.config.ts) reaches across the workspace to do it:
...
living outside these globs renders with its styles silently missing. Any new source directory, in the app or in a
package, needs a matching glob here.

## Theming & Color Tokens

Colors are HSL triplets stored as CSS custom properties in [globals.css](../apps/web/app/globals.css): the light theme
"Aurora Glass" under `:root`, and the dark theme "Midnight Indigo" under `.dark`. Dark is the default. The shared preset
maps every token to a utility through `hsl(var(--token))`, which is why components write `bg-brand` or
`text-muted-foreground` and never a hex value.
...
meta in `layout.tsx`, the OG image, and the brand artwork under `public/brand/`. These are kept in sync by hand.

[docs/design/colors.yml](design/colors.yml) is the source of truth for all of it: every token, its computed hex, where
it is used, and the exact sync rules. Read it before any color work.

## Content Single Source of Truth

Brand strings, the launch schedule, navigation, and contact links live in one file,
[packages/ui/lib/constants.ts](../packages/ui/lib/constants.ts). Components and apps read from it instead of hardcoding
text, which is what keeps the header, the footer, the page metadata, and the structured data agreeing with each other.
...
class rather than by importing anything. [typography.yml](design/typography.yml) carries the full type system, and
[spacing.yml](design/spacing.yml) the layout scale, the container, and the fluid rhythm these sections depend on.
...
real constraint, not a formality, and [docs/components/README.md](components/README.md) records which side each
...
Adding a second app or a shared package follows the existing shape:
  ...
  1. Create the directory under `apps/` or `packages/`. The workspace globs pick it up with no registration step.
...
## Related References
  ...
  * [docs/components/README.md](components/README.md) - the component library: props, variants, anatomy, and
    accessibility, one page per durable component.
  * [docs/design/colors.yml](design/colors.yml) - the color system and the rules for changing it.
  * [docs/design/spacing.yml](design/spacing.yml) - the layout system: scale, container, breakpoints, vertical rhythm,
